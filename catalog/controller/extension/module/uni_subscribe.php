...
class ControllerExtensionModuleUniSubscribe extends Controller {
	...
	public function add() {
		...
		if ($attempts_info && ($attempts_info['total'] >= (int)$uniset['subscribe_attempt']) && strtotime($attempts_info['date_modified']) > strtotime('-1 hour')) {
		...
		if (!$json) {
			...
			$this->model_extension_module_uni_subscribe->addAttempt($customer_ip);
		}	
