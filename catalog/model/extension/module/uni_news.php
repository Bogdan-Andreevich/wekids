...
class ModelExtensionModuleUniNews extends Model {
	...
	public function getNewsStoryRelatedProduct($news_id) {
		...
		$query = $this->db->query("SELECT nr.product_id FROM ".DB_PREFIX."uni_news_product_related nr LEFT JOIN `".DB_PREFIX."product` p ON (nr.product_id = p.product_id) LEFT JOIN `".DB_PREFIX."product_to_store` p2s ON (p.product_id = p2s.product_id) WHERE news_id = '".(int)$news_id."' AND p2s.store_id = '".(int)$this->config->get('config_store_id')."' AND p.status = '1' AND p.date_available <= NOW() LIMIT 10");
	...
	public function getPrevNextNews($news_id, $date_added, $category_id) {
		...
		if($category_id) {
			...
			foreach($types as $key => $type) {
				...
				$sql .= " WHERE n2c.category_id = '".(int)$category_id."' AND n.date_added ".$this->db->escape($type['sign'])." '".$this->db->escape($date_added)."' AND n.status = '1' AND nd.language_id = '".$language_id."' AND n2s.store_id = '".$store_id."'";
	...
	public function getCategoryNews($news_id) {
		$query = $this->db->query("SELECT n2c.category_id FROM `".DB_PREFIX."uni_news_story_to_category` n2c LEFT JOIN `".DB_PREFIX."uni_news_category_path` cp ON (n2c.category_id = cp.category_id) WHERE n2c.news_id = '".(int)$news_id."' ORDER BY cp.level DESC, cp.category_id LIMIT 1");
	...
	public function getPathByCategory($category_id) {
		$query = $this->db->query("SELECT cp.path_id FROM `".DB_PREFIX."uni_news_category_path` cp WHERE cp.category_id = '".(int)$category_id."' ORDER BY cp.level");

		$result = [];

		foreach ($query->rows as $path) {
			$result[] = (int)$path['path_id'];
		}
